    ...
    <AppLayout title="UserProfile">
        <div class="bg-gray-200 ">
            <div class="container flex items-center px-6 py-4 mx-auto overflow-y-auto whitespace-nowrap">
                ...
                <a href="#" class="flex items-center text-green-600 -px-2 hover:underline">
                    <span class="mx-2">
                        {{ usePage().props.value.user.name }}</span>
                </a>
            </div>
        </div>
        ...
        <div style="background: url('https://www.cmu.edu.ph/wp-content/uploads/2022/05/1new-cmubanner-1045x218-1.jpg'); background-repeat: no-repeat; background-size:contain;" class="bg-gray-800 font-sans pt-[30vmin] lg:pt-0 lg:h-[50vmin] w-full flex flex-row justify-center items-center">
            <div class="card w-96 mx-auto bg-white rounded-lg shadow-xl hover:shadow">
                <img class="w-32 mx-auto rounded-full -mt-20 border-8 border-white" :src="
                    usePage().props.value.user
                        .profile_photo_url
                " alt="">
                <div class="text-center mt-2 text-3xl font-medium"> {{ usePage().props.value.user.name }}({{
                ...
                <div class="flex p-4">
                    <div class="w-full text-center">
                        <span class="font-bold"> {{ usePage().props.value.user_posts.length }}</span> Posts
                    </div>
        ...
        <NoPostsYet v-if="usePage().props.value.user_posts.length == 0" />
        <div class=" w-full px-2 lg:px-10  grid grid-cols-1 lg:grid-cols-3 gap-3">

            <article v-for="(posts, key) in usePage().props.value.user_posts" :key="key"
                        ...
                        <div class="flex items-start px-4 py-6">
                            ...
                            <div class="">
                                <div class="flex items-center justify-between">
                                    <h2 class="text-lg font-semibold text-gray-900 -mt-1">{{ commentsData.user.name }}
                                    </h2>
                         
                                </div>
                                <small class="text-gray-700">Commented  {{ date_conversion2(commentsData.updated_at) }} </small>
