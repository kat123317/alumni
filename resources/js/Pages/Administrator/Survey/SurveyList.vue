...
const showEditModal = (index, method = 'edit') => {
    ...
    if (method == 'edit') {
        modals.add_edit.details.title = 'Update Survey: '+ edit_survey.name;
        modals.add_edit.details.btn_submit = 'Submit';
    } else {
        modals.add_edit.details.title = 'Send Invites from Survey: '+ edit_survey.name;
...
const showDeleteModal = (index) => {
    ...
    modals.delete.details.content = 'Are you sure you want to delete '+usePage().props.value.surveys.data[index].name + '?'
...
<template>
    <div class="px-2">
        ...
        >
            ...
            >
                ...
                >
                    ...
                    >
                        <Link :href="route('surveys.questions.index', {survey_id: survey.id})">
                            {{ survey.name }}
                        </Link>
                    </th>
                    <td
                        class="py-4 capitalize px-6"
                    >
                        {{ survey.status }}
                    </td>
                    <td class="py-4 px-6">
        ...
        <div class="px-4 w-100 py-3 flex items-center justify-center border-gray-200 sm:px-6">
            <Pagination v-bind:links="$page.props.surveys.links" :search="search" />
        </div>
    </div>
</template>