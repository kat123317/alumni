  ...
  <section class="text-gray-600 body-font relative">
    <div v-if="alertOn" class="bg-green-100 alertanim text-center py-4 lg:px-4">
      <div class="p-2 bg-green-800 items-center text-green-100 leading-none lg:rounded-full flex lg:inline-flex"
        ...
        <span class="font-semibold mr-2 text-left flex-auto">Course Successfully added</span>
    ...
    <div v-if="alertOnUpdate" class="bg-blue-100 alertanim text-center py-4 lg:px-4">
      <div class="p-2 bg-blue-800 items-center text-blue-100 leading-none lg:rounded-full flex lg:inline-flex"
        ...
        <span class="font-semibold mr-2 text-left flex-auto">Course Successfully updated</span>
        <svg class="fill-current opacity-75 h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
          <path d="M12.95 10.707l.707-.707L8 4.343 6.586 5.757 10.828 10l-4.242 4.243L8 15.657l4.95-4.95z" />
        </svg>
      </div>
    ...
    <div v-if="alertOnDelete" class="bg-red-100 alertanim text-center py-4 lg:px-4">
      <div class="p-2 bg-red-800 items-center text-red-100 leading-none lg:rounded-full flex lg:inline-flex"
        ...
        <span class="font-semibold mr-2 text-left flex-auto">Course Successfully deleted</span>
        <svg class="fill-current opacity-75 h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
          <path d="M12.95 10.707l.707-.707L8 4.343 6.586 5.757 10.828 10l-4.242 4.243L8 15.657l4.95-4.95z" />
        </svg>
      </div>
        ...
        <div class="flex flex-wrap -m-2">
          <div class="p-2 w-1/3">
            <div class="relative">
              ...
              <input v-model="add_course_data.abbreviation" type="text" id="name" name="name"
          ...
          <div class="p-2 w-full">
            <button @click="course_save()"
              class="flex mx-auto text-white bg-green-500 border-0 py-2 px-8 focus:outline-none hover:bg-green-600 rounded text-lg">Submit</button>
          </div>
          <div class="p-2 w-full pt-8 mt-8 border-t border-gray-200 text-center">
            ...
            <div class="overflow-x-auto  relative shadow-md sm:rounded-lg">
              <div class="p-2 p-2 w-1/2">
                <div class="relative">
                  <select v-model="select_course_view.id" @change="filter_table_show()" id="countries"
                    class="bg-gray-50 border border-gray-300 w text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
              ...
              <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                  <tr v-for="(courses, key) in usePage().props.value.courses.data" :key="key">
                    ...
                    <td class="py-4 px-6">
                      <a href="#"
                        @click="function_open_update_modal(courses.id, courses.name, courses.abbreviation, courses.college_id, courses.logo)"
                        class="font-medium text-green-600 hover:underline">Edit</a>
                      <a href="#" @click="function_open_delete_modal(courses.id)"
                        class="font-medium ml-2 text-red-600 hover:underline">Delete</a>
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div class="relative bg-white rounded-lg shadow">
              ...
              <div class="p-6 text-center">
                ...
                <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure you want to delete
                  this course?</h3>
                <button @click="function_delete_event()" data-modal-toggle="popup-modal" type="button"
            ...
            <div class="relative bg-white rounded-lg shadow">
              <div class="p-2 w-full">
                <div class="relative">
                  ...
                  <input v-model="update_course_data.abbreviation" type="text" id="title" name="title"
                    class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-green-500 focus:bg-white focus:ring-2 focus:ring-green-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"
                    required>
                </div>
              </div>
              <div class="p-2 w-full">
                <div class="relative">
                  ...
                  <textarea v-model="update_course_data.name" id="content" rows="4"
              ...
              <div class="p-2 w-full">
                <div class="relative">
                  ...
                  <label for="content"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">College</label>
                  <select v-model="update_course_data.college_id" id="countries"
                    class="bg-gray-50 border border-gray-300 w text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                    <option v-for="(colleges, key) in usePage().props.value.colleges" :key="key" :value=colleges.id>
                      {{colleges.name}}</option>
                  </select>
              ...
              <div class="p-2 w-full">
                <div class="relative">
                  ...
                  <input v-model="update_course_data.image_name" type="email" id="email" name="email"
                    class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-green-500 focus:bg-white focus:ring-2 focus:ring-green-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"
                    disabled>
                  <input id="course-hidden-edit" type="file" class="hidden" accept="image/png, image/gif, image/jpeg" />
                  <button @click="openFileEdit"
                    class="mt-2 rounded-sm px-3 py-1 bg-gray-200 hover:bg-gray-300 focus:shadow-outline focus:outline-none">
                    Select Logo
                  </button>
                </div>
              ...
              <div class="p-6 text-center">
                ...
                <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure you want to update
                  this course?</h3>
                <button @click="function_update_course()" data-modal-toggle="popup-modal" type="button"
                  class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                  Yes, I'm sure
                </button>
